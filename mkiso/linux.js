...
module.exports = function (opts, cb) {
  ...
  if (fs.existsSync(opts.foldername)) {
    exec(`grub-mkrescue -o ${opts.filename} ${opts.foldername}`, (code, out) => {
      cb();
    });
  } else {
    exec(`mkdir ${opts.foldername}`, (e) => {
      if (e) return cb(e);
      ...
      exec(`cp ${opts.kernel} ${opts.foldername}/kernel`, (e) => {
        if (e) return cb(e);

        exec(`cp ${opts.initrd} ${opts.foldername}/initrd`, (e) => {
          exec(`mkdir -p ${opts.foldername}/boot/grub/`, (e) => {
            const grubcfg = 'set timeout=0\nset default=0\n\nmenuentry "JsOS" {\n\tmultiboot /kernel\n\tmodule /initrd\n\tboot\n}\n';
            fs.writeFile(`${opts.foldername}/boot/grub/grub.cfg`, grubcfg, (e) => {
              if (e) return cb(e);
